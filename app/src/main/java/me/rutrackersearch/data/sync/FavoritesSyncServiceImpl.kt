...
import me.rutrackersearch.data.workers.SyncFavoritesWorker
...
class FavoritesSyncServiceImpl @Inject constructor(
    ...
    override suspend fun setSyncPeriod(syncPeriod: SyncPeriod) {
        if (syncPeriod == SyncPeriod.OFF) {
            ...
            val workRequest = periodicWorkRequest<SyncFavoritesWorker>(syncPeriod)
            workManager.enqueueUniquePeriodicWork(workName, REPLACE, workRequest)
        }
    }

    private companion object {
        const val workName = "FavoritesSync"
    }
}
