...
## 10. Открытые вопросы к согласованию

1. **Koin Annotations (KSP)** для domain/features — да/нет? (Меньше boilerplate, но +KSP-зависимость и магия.)
2. **Гранулярность PR**: data+domain одним PR (меньше мостов) или раздельно (мельче диффы)?
3. **`@HiltWorker`-воркеры**: мигрировать в Ф6 или отложить (work:impl всё равно ❌ для KMP — WorkManager)?
4. Нужен ли промежуточный «оба контейнера живут» период в проде, или big-bang влить одним релизом после полного прохода?
